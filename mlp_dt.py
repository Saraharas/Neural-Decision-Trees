...
class DecisionTreeClassifier():
    def __init__(self, X=None, y=None, depth=1, min_leaf=10, mlp_features_ratio=0.3):
        self.X = X
        self.y = y
    ...
    def fit(self, X, y, inner_model=MLPClassifier(solver='lbfgs', hidden_layer_sizes=(50,)), 
        ...
        if X is not None:
            ...
            self.mlp_features_indices = np.array([0, 1])
        if X.shape[0] == 0:
            return
        ...
        self.lhs = DecisionTreeClassifier(X_left, y_left, self.depth - 1, self.min_leaf, self.mlp_features_ratio)
        self.rhs = DecisionTreeClassifier(X_right, y_right, self.depth - 1, self.min_leaf, self.mlp_features_ratio)
